...
# Найти триплеты и просто выводить их на экран. Триплетом называются три числа, которые в сумме дают .
# (решение будет долгим, ибо является демонстрационным решением при теме многопоточного программироаания.)

with open('1Kints.txt', 'r') as data:
    Mylist = data.readlines()
    Mylist.pop()
